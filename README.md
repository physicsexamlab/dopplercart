...
Arduino UF2 ブートローダの double-reset 検出機構を使う方法です。
...
LED のタイミングがうまく取れない場合は、ジャンパ線で強制的にブートローダに入れる方法も使えます。

1. USB ケーブルをボードから抜く
2. ジャンパ線で **B0 ピンと GND ピンを短絡**
3. 短絡したまま **USB ケーブルを差す**
...
- **macOS：** Terminal で `ls /dev/cu.*` を実行し、新しい短い名前のポート（例：`cu.usbmodem101`）が現れているか確認
- **Linux：** Terminal で `ls /dev/ttyACM*` を実行し、新しいポートが現れているか確認
